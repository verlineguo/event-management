...
@section('content')
                ...
                <form method="GET" action="{{ route('member.event.search') }}" id="searchForm">
                    ...
                    <div class="filter-group">
                        ...
                        <select id="fee_type" name="fee_type" class="form-select">
                            ...
                            <option value="free" {{ request('fee_type') === 'free' ? 'selected' : '' }}>Free Events
                            </option>
                            <option value="paid" {{ request('fee_type') === 'paid' ? 'selected' : '' }}>Paid Events
                            </option>
                        </select>
                    </div>
                            ...
                            <div class="event-card ftco-animate" data-category="{{ strtolower($event['category']) }}"
                                ...
                                <div class="event-poster">
                                    ...
                                    <div class="event-date-badge">
                                        <div class="day">
                                            {{ \Carbon\Carbon::parse($event['display_date'])->format('d') }}
                                        </div>
                                        <div class="month">
                                            {{ \Carbon\Carbon::parse($event['display_date'])->format('M') }}
                                        </div>
                                    </div>
    ...
    <script>
        ...
        function clearFilters() {
            // Fixed: Use the correct form ID 'searchForm' instead of 'eventFilterForm'
        ...
        document.getElementById('fee_type').addEventListener('change', filterEvents);
        ...
        function filterEvents() {
            ...
            eventCards.forEach(card => {
                ...
                // Get the actual category name from the event-meta section instead of data attribute
                const categoryElement = card.querySelector('.event-meta .fas.fa-tag').parentElement.querySelector(
                    'span');
                const cardCategoryName = categoryElement ? categoryElement.textContent.trim() : '';
                ...
                // Compare with the selected category name from dropdown
                let matchesCategory = !category;
                if (category) {
                    const selectedCategoryOption = document.querySelector(`#category option[value="${category}"]`);
                    const selectedCategoryName = selectedCategoryOption ? selectedCategoryOption.textContent
                    .trim() : '';
                    matchesCategory = cardCategoryName === selectedCategoryName;
                }
            ...
            if (resultsCountElement) {
                resultsCountElement.innerHTML = `<strong>${visibleCount}</strong> events found`;
            }
        }
